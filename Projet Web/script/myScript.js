...
var level_0, level_1, level_2, level_3, level_4, level_5, level_6, level_7, level_8, level_9;
...
function Projectile(evilness,x,y,xSpeed,ySpeed,dir,width,height) //evilness : 0 hero, 1 ennemi ;dir : 0 horizontal, 1 vertical
{
    ...
    this.update = function()
	{
        if(this.exists)
		{
			...
			if(this.x > ArenaWidth || this.x < 0 || this.y > ArenaHeight || this.y < 0)
			{
				this.exits = false;
			}
        }
...
function Enemy(x,y,speed,dir,type) //dir = 0:horizontal, 1: vertical
{
    ...
    this.fire = function ()
	{
		if(this.type == 10)
		{
			...
			//tir haut-gauche
			tmp = new Projectile(1,this.x - 5,this.y - 5,-shootSpeed,-shootSpeed,this.dir,5,5);
			this.projectileSet.add(tmp);
			
			//tir haut-droit
			tmp = new Projectile(1,this.x + this.width + 5,this.y - 5,shootSpeed,-shootSpeed,this.dir,5,5);
			this.projectileSet.add(tmp);
			
			//tir bas-gauche
			tmp = new Projectile(1,this.x - 5,this.y + this.height + 5,-shootSpeed,shootSpeed,this.dir,5,5);
			this.projectileSet.add(tmp);
			
			//tir bas-droit
			tmp = new Projectile(1,this.x + this.width + 5,this.y + this.height + 5,shootSpeed,shootSpeed,this.dir,5,5);
			this.projectileSet.add(tmp);
		}		
		else
		{
...
function updateItems() //mise à jour des éléments
{
    ...
    if(tics % timeNewEnemy == 1 && nbEnemy< nbEnemyMax) //si il est temps de créer un nouvel ennemi et que le nombre d'ennemis max n'est pas atteint
	{
		...
		} while( randx < player.x + player.width + 20
				&& randx > player.x - 60
				&& randy < player.y + player.height +20
				&& randy > player.y - 60)
		...
		else if(randType < level_1)
		{
			new_type = 1;
		}
		else if(randType < level_2)
		{
			new_type = 2;
		}
		else if(randType < level_3)
		{
			new_type = 3;
		}
		else if(randType < level_4)
		{
			new_type = 4;
		}
		else if(randType < level_5)
		{
			new_type = 5;
		}
		else if(randType < level_6)
		{
			new_type = 6;
		}
		else if(randType < level_7)
		{
			new_type = 7;
		}
		else if(randType < level_8)
		{
			new_type = 8;
		}
		else if(randType < level_9)
		{
			new_type = 9;
		}
		else
...
function muteMusic()	//activer ou désactiver le son
{
	...
	else 
	{
		...
		audioTrack.volume = 0.1;	//on active le son
		soundGame = true;
	}
